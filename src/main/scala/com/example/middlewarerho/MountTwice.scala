...
object MountTwice extends StreamApp {
  val port: Int = envOrNone("HTTP_PORT").fold(8080)(_.toInt)

  val swaggerSupport: RhoMiddleware = SwaggerSupport()

  val services: HttpService = (Service1.service and Service2.service).toService(swaggerSupport)

  def stream(args: List[String]): fs2.Stream[Task, Nothing] = BlazeBuilder.bindHttp(port)
    .mountService(Service.withFallback(StaticContentService.routes)(services)) // service 1 is exposed but not wrapped with Middleware
    .mountService(Middleware.addHeader(Service1.service.toService(swaggerSupport), Header("SomeKey", "SomeValue")))
    .serve
  ...
  object Middleware {
    def addHeader(service: HttpService, header: Header): HttpService = Service.lift { req =>
      service(req).map {
